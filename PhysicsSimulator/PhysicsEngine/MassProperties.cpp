...
// Desc: 
// Implements method given in "MassProperties.h"
// In particular computes mass and inertia tensor from density
//
// Adapted from Brian Mirtich's volInt.c, see copywright below.
...
void readPolyhedron(ifstream &meshFile, Polyhedron *p)
{
    string token;
    int i, j;
    ...
    //initialize array of verticies
    meshFile >> token;
    p->numVerts = stoi(token);
    p->verts = new double*[p->numVerts];
    for (int i = 0; i < p->numVerts; i++) {
        p->verts[i] = new double[3];
    }

    //initialize array of faces
    meshFile >> token;
    p->numFaces = stoi(token);
    p->faces = new Face[p->numFaces];

    //set values of verticies
    for (i = 0; i < p->numVerts; i++) {
        meshFile >> token;
        p->verts[i][X] = stod(token);
        meshFile >> token;
        p->verts[i][Y] = stod(token);
        meshFile >> token;
        p->verts[i][Z] = stod(token);
    }

    //set face/vertex pairings
    for (i = 0; i < p->numFaces; i++) {
        f = &p->faces[i];
        f->poly = p;
        for (j = 0; j < f->numVerts; j++) {
            meshFile >> token;
            f->verts[j] = stoi(token);
        }
    ...
    double density, double *massProperties)
{
    ...
    */
    ...
    //copy overall mass into mass properties
    mass = density * T0;
    massProperties[9] = mass;
    ...
    */
    ...
    //copy tensor into mass properties
    massProperties[0] = J[X][X];
    massProperties[1] = J[X][Y];
    massProperties[2] = J[X][Z];
    massProperties[3] = J[Y][X];
    massProperties[4] = J[Y][Y];
    massProperties[5] = J[Y][Z];
    massProperties[6] = J[Z][X];
    massProperties[7] = J[Z][Y];
    massProperties[8] = J[Z][Z];
}
